...
while ((k_error_last-k_error) > 0.1) or k_error < 0 or k_error_last < 0:
...
rw_add = np.column_stack((rw,feature_k.reshape(len(feature_k),1)))
...
for index in range(num_i):
    ...
    #use linear regression to find model
    w_new = np.linalg.inv(np.transpose(x_train)@x_train)@np.transpose(x_train)@y_train

    #record average squared error
    sq_err += sum(pow(x_test@w_new-y_test,2))/len(y_test)
sq_err = sq_err/num_i

print("New Squared Error: ",sq_err)
print("New Weights: \n",w_new)
...
#Try again using regularization
